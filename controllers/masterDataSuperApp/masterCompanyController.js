...
export const createCompany = async (req, res) => {
  const { company_code, company_name, address = "", is_active = 1 } = req.body;
  ...
  try {
    ...
    const [r] = await safeQuery(
      `INSERT INTO mst_company (company_code, company_name, address, is_active) VALUES (?, ?, ?, ?)`,
      [company_code.trim().toUpperCase(), company_name.trim(), address.trim(), is_active ? 1 : 0]
    );
...
export const updateCompany = async (req, res) => {
  const { company_code, company_name, address, is_active } = req.body;
  if (!company_code?.trim()) return res.status(400).json({ message: "Kode company wajib diisi" });
  if (!company_name?.trim()) return res.status(400).json({ message: "Nama company wajib diisi" });
  try {
    ...
    if (exist.length) return res.status(409).json({ message: "Kode company sudah digunakan" });
    await safeQuery(
      `UPDATE mst_company SET company_code = ?, company_name = ?, address = ?, is_active = ? WHERE company_id = ?`,
      [company_code.trim().toUpperCase(), company_name.trim(), (address || "").trim(), is_active ? 1 : 0, req.params.id]
    );
