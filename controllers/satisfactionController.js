...
export async function checkSurveyStatus(req, res) {
  try {
    ...
    if (!employeeId) {
      return res.status(401).json({ message: "Employee ID not found in session" });
    }
...
export async function getSurveyMasterData(req, res) {
  try {
    const [companies]  = await safeQuery(`SELECT company_id, company_name FROM mst_company WHERE is_active = 1 ORDER BY company_name`);
    const [departments]= await safeQuery(`SELECT department_id, department_name FROM mst_department WHERE is_active = 1 ORDER BY department_name`);
    const [employees]  = await safeQuery(`SELECT join_date FROM mst_employee`);

    const now       = new Date();
    const tenureSet = new Set();

    employees.forEach(emp => {
      const diffMonth = (now - new Date(emp.join_date)) / (1000 * 60 * 60 * 24 * 30.44);
      if      (diffMonth < 3)  tenureSet.add("< 3 bulan");
      else if (diffMonth < 6)  tenureSet.add("3 - 6 bulan");
      else if (diffMonth < 12) tenureSet.add("6 - 12 bulan");
      else                     tenureSet.add("> 1 tahun");
    });
...
export async function submitSurvey(req, res) {
  // Transaction tetap pakai pool.getConnection langsung
  ...
  try {
    ...
    if (existing.length > 0) {
      return res.status(400).json({ message: "Anda sudah mengisi survei untuk periode ini" });
    }

    const {
      full_name, company_name, department_name, job_level_name, tenure,
    ...
    if (!full_name || !department_name || !job_level_name || !tenure || !overall_satisfaction) {
      return res.status(400).json({ message: "Mohon lengkapi semua informasi umum dan kepuasan kerja" });
    }
    ...
    for (let i = 0; i < likertFields.length; i++) {
      if (likertFields[i] !== null && likertFields[i] !== undefined) {
        ...
        if (isNaN(val) || val < 1 || val > 5) {
          return res.status(400).json({ message: "Nilai penilaian aspek kerja harus antara 1-5" });
        }
      }
    }
    ...
    await connection.query(
      ...
      [
        employeeId, full_name, company_name || null, department_name || null,
        job_level_name || null, tenure, overall_satisfaction,
    ...
    await connection.query(
      `INSERT INTO tr_employee_satisfaction_audit (employee_id, full_name, survey_key, status)
       VALUES (?, ?, ?, 'COMPLETED')`,
      [employeeId, full_name, surveyKey]
    );

    await connection.commit();
    res.json({ message: "Terima kasih! Survei berhasil dikirim.", surveyKey });
  } catch (err) {
...
export async function getSurveyStats(req, res) {
  try {
    ...
    const [satisfactionDist] = await safeQuery(
    ...
    const [avgScores] = await safeQuery(
      `SELECT 
